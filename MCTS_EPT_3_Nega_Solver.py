...
from MCTS_EPT_3_Node import MCTSEPT3Node
...
class MCTSEPT3(object):
    ...
    def __init__(self, board, **kwargs):
        ...
        # pass
        print("\n")
        print("******* MCTS-EPT Nega Solver Object Created *******")
        print("\n")
    ...
    def run_selection(self, node):
        ...
        while not(node.is_leaf):
            ...
            if node.sims == 0:  # only for root node
                ...
                max_selection = -math.inf
                selection_score = -math.inf
                log_value = log(node.sims)
                for each in node.children:
                    if each.sims == 0:
                        selection_score = 99999
                        # selection_score = (
                        #     (1-self.alpha)*99999 + self.alpha*each.eval) + (c_value * 99999)
                    else:
                        # UCT
                        # selection_score = ((each.score) + (c_value *
                        ...
                        # selection_score = each.score + c_value * sqrt((log_value/each.sims) * min(1/4, (each.score * (1 - each.score) +
                        #                                                                                 sqrt(2*log_value/each.sims))))
                        # Implicit
                        selection_score = ((1-self.alpha)*each.score + self.alpha*each.eval) + (c_value *
                                                                                                sqrt(log_value/each.sims))

                    if selection_score > max_selection:
                        max_selection = selection_score
                        node = each
        return node
    ...
    def run_simulation(self, node):
        ...
        for move in range(self.terminal_depth):
            ...
            if board_state.is_game_over():
                if board_state.is_checkmate():  # assign winner only if checkmate
                    if board_state.turn == self.original_player:  # original player lost
                        if node.depth % 2 == 0:  # return win at opponent depth
                            return 1.0
                        else:  # return loss at player depth
                            return 0.0
                    else:  # original player won
                        # return 1.0
                        if node.depth % 2 == 0:  # return loss at opponent depth
                            return 0.0
                        else:  # return win at player depth
                            return 1.0
                return 0.0  # is draw considered a loss for mcts-ept?
        ...
        if self.original_player:  # if player is white
            if node.depth % 2 == 0:  # use black eval at opponent depth
                score = 1 / (1 + (10 ** -(-stat_eval / 400)))
            else:
                score = 1 / (1 + (10 ** -(stat_eval / 400)))
        else:  # if player is black
            if node.depth % 2 == 0:  # use white eval at opponent depth
                score = 1 / (1 + (10 ** -(stat_eval / 400)))
            else:
                score = 1 / (1 + (10 ** -(-stat_eval / 400)))
    ...
    def run_backpropagation(self, node, result):
        node.winsum += result
        node.sims += 1
        evaluation = node.eval  # store eval of node to be back propagated
        side = node.depth % 2
        while(node.parent):
            node = node.parent
            if node.depth % 2 == side:  # update values only if on the same side
                node.winsum += result
                node.sims += 1
                if node.eval < evaluation:
    ...
    def algo_init(self):
        ...
        globals()[str(self.starting_board_state.fen())+str(0)] = MCTSEPT3Node(state=str(
    ...
    def algo_render(self):
        ...
        #           node.sims, round(node.score, 2), node.eval)
        # print("\n")
